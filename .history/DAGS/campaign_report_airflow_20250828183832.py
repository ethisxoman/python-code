...
# Query that checks for new rows
check_new_rows = """
SELECT COUNT(*) 
...
with DAG(
    ...
    schedule_interval="@once",  # Start once, then sensor controls it
    catchup=False,
) as dag:

    # Sensor: waits until new rows appear
    wait_for_new_rows = SqlSensor(
        task_id="wait_for_new_campaigns",
        conn_id="reporting_mysql",  # <-- create this Airflow connection for your reporting DB
        sql=check_new_rows,
