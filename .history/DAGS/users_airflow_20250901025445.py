...
def extract_and_load_func(**dictionary):
    ...
    try:
        src_cur.execute(query)
        rows = src_cur.fetchall()
        columns = [desc[0] for desc in src_cur.description] #get column names
        print(f"Query executed. Rows fetched: {len(rows)}")
    except Exception as e:
        print(f"[ERROR] Failed executing query: {e}")
        src_conn.close()
    ...
    df = pd.DataFrame(rows, columns=columns)  #assign proper column names
    print(f"DataFrame created with {len(df)} rows and {len(df.columns)} columns.")
    ...
    table_columns = [row["Field"] for row in dest_cur.fetchall()]
