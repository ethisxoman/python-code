...
#     catchup=False,
#     default_args={
#         "execution_timeout": timedelta(minutes=30),  # prevent premature kill
#         "retries": 1,
...
def extract_and_load_func(**dictionary):
    ...
    dest_cur.execute(f"SHOW COLUMNS FROM {table_name}")
    col_result = dest_cur.fetchall()

    # Handle tuple vs dict cursor
    if isinstance(col_result[0], tuple):
        table_columns = [row[0] for row in col_result]
    else:  # dict-style
        table_columns = [row["Field"] for row in col_result]

    df = df[[c for c in df.columns if c in table_columns]]
    print(f"Filtered DataFrame: {len(df.columns)} columns match the table schema.")

    if df.empty:
        print("No matching columns between DataFrame and table. Skipping insert.")
        return
    ...
    # Replace NaN with None in one go
    rows_to_insert = df.where(pd.notnull(df), None).values.tolist()

    # ✅ Bulk insert
    dest_cur.executemany(insert_query, rows_to_insert)

    dest_conn.commit()
    print(f"Inserted {dest_cur.rowcount} rows into {table_name}.")
