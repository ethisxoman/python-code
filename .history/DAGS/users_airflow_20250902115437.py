...
def row_to_hash(row):
    str_row = "|".join([str(v) if v is not None else "" for v in row])
    return hashlib.md5(str_row.encode()).hexdigest()
...
def extract_and_load_func(**dictionary):
    ...
    CREATE TABLE IF NOT EXISTS {table_name} (
        ...
        row_hash CHAR(32),   -- NEW
        UNIQUE KEY uniq_user_hash (user_id, row_hash) -- prevent exact duplicates
    );
    """
    ...
    columns.append("row_hash")  # add the hash column
    col_names = ", ".join([f"`{col}`" for col in columns])
    placeholders = ", ".join(["%s"] * len(columns))
    update_clause = ", ".join([f"{col}=VALUES({col})" for col in columns if col != "id"])  # don't update PK
...
"""
    ...
    for _, row in df.reset_index(drop=True).iterrows():
        ...
        # Compute hash from the row values
        row_hash = row_to_hash(list(dict_row.values()))

        # Add hash to dict_row
        dict_row["row_hash"] = row_hash

        values.append([dict_row.get(col) for col in columns] + [row_hash])
