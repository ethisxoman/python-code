...
def extract_and_load_upsert(**dictionary):
    ...
    # Get count before insertion (safe for both tuple/dict cursor)
    dest_cur.execute(f"SELECT COUNT(*) as cnt FROM {table_name}")
    row = dest_cur.fetchone()
    count_before = list(row.values())[0] if isinstance(row, dict) else row[0]
    ...
    # Get count after insertion (safe for both tuple/dict cursor)
    dest_cur.execute(f"SELECT COUNT(*) as cnt FROM {table_name}")
    row = dest_cur.fetchone()
    count_after = list(row.values())[0] if isinstance(row, dict) else row[0]
