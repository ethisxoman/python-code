...
query = """SELECT 
    ...
    cntrs.name AS country,
    GROUP_CONCAT(DISTINCT c.type) AS user_type,
    a.aml_record,
...
FROM ethisx_accounts.users AS u
LEFT JOIN ethisx_accounts.countries AS cntrs ON u.country_id = cntrs.id
LEFT JOIN ethisx_accounts.user_types AS b ON u.id = b.user_id
LEFT JOIN ethisx_accounts.types AS c ON b.type_id = c.id
LEFT JOIN ethisx_accounts.kyc_infos AS k ON u.id = k.namescan_id
LEFT JOIN ethisx_accounts.kyc_ex_platforms AS ke ON k.id = ke.kyc_info_id
LEFT JOIN ethisx_accounts.aml_stores AS a ON u.id = a.id
LEFT JOIN ethisx_accounts.investor_categories AS ic ON u.investor_category_id = ic.id
LEFT JOIN ethisxadmin.user_bank_infos AS ub ON u.id = ub.user_id    
WHERE u.id IS NOT NULL
...
def extract_and_load_func(**kwargs):
    ...
    try:
        ...
        print(f"[ERROR] Source DB connection failed: {e}")
        return
    ...
    try:
        ...
        print(f"Fetched {len(rows)} rows with {len(colnames)} columns.")
    except Exception as e:
    ...
    try:
        ...
        print(f"[ERROR] Reporting DB connection failed: {e}")
        return
    ...
    col_names = ", ".join([f"`{col}`" for col in columns])
    placeholders = ", ".join(["%s"] * len(columns))
    insertion = f"""INSERT INTO {table_name} ({col_names})
                    VALUES ({placeholders})
    ...
    for _, row in df.iterrows():
        ...
        if len(values) >= batch_size:
            dest_cur.executemany(insertion, values)
            dest_conn.commit()
            print(f"Inserted batch of {len(values)} rows.")
            values.clear()
    ...
    dest_cur.close()
    dest_conn.close()
    print(f"✅ Insert completed: {len(df)} rows into {table_name}.")
