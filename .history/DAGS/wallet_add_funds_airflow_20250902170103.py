...
# Updated query to get DISTINCT records and avoid duplicates from multiple user types
query = """
    SELECT DISTINCT
        fu.user_id, 
    ...
    LEFT JOIN ethisx_wallet_live.add_funds AS a	
        ON fu.fund_id = a.id
    WHERE a.id IS NOT NULL  -- Only get records with actual add_funds data
    ORDER BY u.id, a.id
"""
...
def extract_and_load_func(**dictionary):
    ...
    # Updated table with proper unique constraint
    create_table = f"""
    CREATE TABLE IF NOT EXISTS {table_name} (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        ...
        -- Proper unique constraint: one record per user per transaction
    ...
    print(f"- Total records processed: {inserted_count}")
    print("Reporting DB connection closed.")

# Alternative UPSERT approach
def extract_and_load_upsert(**dictionary):
    """UPSERT approach - handles updates to existing transactions"""
    ...
    print("Removing duplicates within source data...")
...
def extract_and_load_incremental(**dictionary):
    """Incremental approach - only inserts truly new records"""
    print("Starting incremental wallet pipeline...")
    ...
    # Simple INSERT IGNORE for new records only
    inserted_count = 0
    for _, row in df.reset_index(drop=True).iterrows():
        dict_row = row.to_dict()
...
# DAG with improved configuration
with DAG(
    ...
    # Recommended: Use UPSERT approach
    task = PythonOperator(
    ...
    # Alternative: Use incremental approach
    # Uncomment this and comment above if you prefer incremental loading
    # task = PythonOperator(
