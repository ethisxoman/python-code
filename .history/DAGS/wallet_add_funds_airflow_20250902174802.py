...
query = """
    ...
    LEFT JOIN ethisx_wallet_live.add_funds AS a	
    ON fu.fund_id = a.id
    WHERE a.updated_at > %s  -- Only get records updated since last run
    ORDER BY u.id
"""

def extract_and_load_func(**context):
    print("Connecting to SOURCE database...")
    
    # Get the last execution time to only fetch new/updated records
    last_execution_date = context['execution_date'] if 'execution_date' in context else days_ago(1)
    last_execution_str = last_execution_date.strftime('%Y-%m-%d %H:%M:%S')
    ...
    print("Executing source query...")
    try:
        # Pass the last execution time as parameter
        src_cur.execute(query, (last_execution_str,))
        rows = src_cur.fetchall()
        columns = [desc[0] for desc in src_cur.description]
        df = pd.DataFrame(rows, columns=columns)
        print(f"DataFrame created with {len(df)} rows and {len(df.columns)} columns: {df.columns.tolist()}")
    ...
    if len(df) == 0:
        print("No new rows found in source database since last run.")
        return

    # Load to reporting database
    print("Connecting to REPORTING database...")
    try:
        dest_conn = get_airflow_connection(reporting_db)
        dest_cur = dest_conn.cursor()
    ...
    CREATE TABLE IF NOT EXISTS {table_name} (
        ...
        add_funds_updated_amount DECIMAL(15,2),
        add_funds_date DATE,
        add_funds_payment_type VARCHAR(50),  -- Changed from BIGINT to VARCHAR to match your CASE statement
        add_funds_transaction_id VARCHAR(255) NOT NULL,
        add_funds_status VARCHAR(255),            
        add_funds_created_at TIMESTAMP,
        add_funds_updated_at TIMESTAMP,
        UNIQUE KEY unique_transaction (add_funds_transaction_id),
        INDEX idx_updated_at (add_funds_updated_at)  -- Added index for better performance
    );
    """
    dest_cur.execute(create_table)
...
with DAG(
    ...
    task = PythonOperator(
        task_id="extract_and_load",
        python_callable=extract_and_load_func,
        provide_context=True,  # This allows access to execution_date
    )
