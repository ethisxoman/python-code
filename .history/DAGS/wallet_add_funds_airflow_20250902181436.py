...
query = """
    ...
    FROM ethisx_accounts.users u 
    INNER JOIN ethisx_wallet_live.fund_users fu ON u.id = fu.user_id
    LEFT JOIN ethisx_wallet_live.add_funds AS a ON fu.fund_id = a.id
    ORDER BY u.id
"""
...
def extract_and_load_func(**dictionary):
    ...
    CREATE TABLE IF NOT EXISTS {table_name} (
        ...
        add_funds_updated_amount DECIMAL(15,2),
        add_funds_date DATETIME,

        add_funds_payment_type VARCHAR(50),

        add_funds_transaction_id VARCHAR(255) NOT NULL,
        add_funds_status VARCHAR(255),            
        add_funds_created_at TIMESTAMP NULL,
        add_funds_updated_at TIMESTAMP NULL
    );
    """
    dest_cur.execute(create_table)
    ...
    col_names = ", ".join([f"`{col}`" for col in columns])
    placeholders = ", ".join(["%s"] * len(columns))
    insertion = f"INSERT INTO {table_name} ({col_names}) VALUES ({placeholders})"

    # Replace NaN with None
    values = []
    for _, row in df.iterrows():
        clean_row = [
            None if (pd.isna(v) or (isinstance(v, float) and math.isnan(v))) else v
            for v in row.tolist()
        ]
...
with DAG(
    dag_id="wallet_add_funds_airflow",
    start_date=days_ago(1),      # starts yesterday
    schedule_interval="0 */6 * * *",   
    catchup=False,
) as dag:
