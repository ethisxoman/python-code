...
def extract_and_load_func(**dictionary):
    ...
    withdraw_funds_status VARCHAR(50),
    ...
    dest_cur.execute(f"ALTER TABLE {table_name} MODIFY COLUMN withdraw_funds_payment_type VARCHAR(50)")
    dest_cur.execute(f"ALTER TABLE {table_name} MODIFY COLUMN withdraw_funds_status VARCHAR(50)")
    ...
    for _, row in df.reset_index(drop=True).iterrows():
        ...
        withdraw_funds_amount = VALUES(withdraw_funds_amount),
        withdraw_funds_status = VALUES(withdraw_funds_status),
        withdraw_funds_updated_at = VALUES(withdraw_funds_updated_at)
        """  
